...
private:
    ...
    void stop() {
        // Workers drain accepted tasks before shutdown returns.
        std::unique_lock<std::mutex> lock(mutex_);
        stopping_ = true;
        lock.unlock();
        ready_.notify_all();
...
struct TargetArtifact {
    ...
    std::shared_ptr<Implementation> load(const KernelPlan& plan) {
        ...
        // Exactly one caller creates the CUDA launcher; every caller shares its result.
        if (promise != nullptr) {
            try {
    ...
    // Shared futures keep the compiled image and loaded launcher alive for this target.
...
private:
    ...
    private:
        // The job keeps its interned kernel alive until compilation finishes.
    ...
    std::shared_ptr<TargetArtifact<Implementation>> get_or_schedule(
        ...
        std::shared_ptr<std::promise<CompiledKernelImage>> promise;
        {
            ...
            // Publish before submission so concurrent callers cannot enqueue duplicates.
...
// The process-wide interner owns one reference; XLA executable states share it.
...
std::shared_ptr<SharedKernel> intern_kernel(
    ...
    // Payload comparison prevents hash collisions from sharing different kernels.
